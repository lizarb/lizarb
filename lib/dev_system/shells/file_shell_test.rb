class DevSystem::FileShellTest < DevSystem::ShellTest
  ...
  test :subject_class, :read_text, true do
    assert subject_class.read_text(__FILE__).size.positive?
  end

  test :subject_class, :read_text, :raise do
    assert_raises ArgumentError do
      subject_class.read_text nil
    end

    assert_raises ArgumentError do
      subject_class.read_text ""
    end

    assert_raises ArgumentError do
      subject_class.read_text "does_not_exist_#{rand 999}"
    end
  end

  #

  test :subject_class, :write_text, :raise do
    assert_raises ArgumentError do
      subject_class.write_text nil, "content"
    end

    assert_raises ArgumentError do
      subject_class.write_text "", "content"
    end
  end

  test :subject_class, :write_text, true do
    path = tmp_dir.join "write_text_#{Time.now.to_i}"
  ...
  test :subject_class, :write_text, :encoding do
    ...
    refute subject_class.exist? path
    assert subject_class.write_text path, s1

    s2 = subject_class.read_text path
  ...
  test :subject_class, :write_text, :create_dir do
    path = tmp_dir.join "write_text_dir_#{Time.now.to_i}/a/b/c"

    refute subject_class.exist? path

    assert_raises Errno::ENOENT do
      assert subject_class.write_text path, "content", create_dir: false
    end

    assert subject_class.write_text path, "content", create_dir: true
