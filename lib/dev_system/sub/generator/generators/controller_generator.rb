class DevSystem::ControllerGenerator < DevSystem::Generator
  ...
  def install_write_box
    @content = render :app_box, format: :rb
  ...
  def install_insert_panel
    lines = render(:install_insert_panel, format: :rb).split("\n").reject(&:empty?)
